...
1. Direct OpenAI: add `OPENAI_API_KEY` as a secret Netlify environment variable, then optionally set
   `AI_DIAGNOSTIC_MODEL=gpt-4o-mini`.
2. Netlify AI Gateway: enable AI for the Netlify site. Netlify supplies `OPENAI_BASE_URL`; do not add
...
pnpm --package=netlify-cli dlx netlify env:set AI_DIAGNOSTIC_MODEL "gpt-4o-mini" --context production
```
